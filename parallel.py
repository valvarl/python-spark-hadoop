...
spark = SparkSession.builder \
    .appName("Forest Cover Type Prediction Parallel") \
    .getOrCreate()
...
def process_row(row):
    data_loaded = [row[feature] for feature in feature_names[:-1]]
    ...
    real_cover_type = row["Cover_Type"]
    error = 1 if int(predicted_cover_type) != int(real_cover_type) else 0
    return error
...
for iter in tqdm.tqdm(range(200)):
    ...
    # Конвертация данных в RDD и использование map для параллельной обработки
    rdd = spark.sparkContext.parallelize(sample_df.to_dict(orient='records'))
    errors = rdd.map(process_row).reduce(lambda x, y: x + y)

    accuracy = (batch_size - errors) / batch_size
...
with open('/output/parallel.csv', 'w') as file:
